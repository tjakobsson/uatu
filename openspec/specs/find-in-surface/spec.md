# find-in-surface Specification

## Purpose

Define uatu's own find — ⌘F — and the routing rule that decides what it acts on.

No engine can scope a host's native find to a subtree, so in a browser ⌘F
matches the document tree, the git log, and the terminal scrollback alongside
the document being read; in UatuCode Desktop WKWebView ships no find bar at
all. Owning find is the only way to scope it, and it closes the desktop gap as
a side effect.

The routing rule is one line — find searches the *active surface* — and the
substance is what "active" means. It is tracked from user interaction and
deliberately not derived from DOM focus, because focus gives the wrong answer
for the most common interaction in the app: clicking a file leaves focus inside
the tree library's shadow root, and a literal focus rule would then search the
sidebar when the reader has just declared interest in a document.
