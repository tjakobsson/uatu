...
The server SHALL report its build identity — version, commit, and an `apiRevision` integer bumped on client/server contract breaks — to authenticated clients via the state payload. The web client SHALL embed its own build identity at bundle time and SHALL compare on boot and on SSE reconnect. The web client SHALL re-establish a permanently-closed event stream itself (browsers close an EventSource for good when its automatic retry fails mid-restart), so a server restart is always eventually observed and the comparison actually runs. On mismatch the web client SHALL reload itself at most once per observed server identity; if the mismatch persists after that reload, the client SHALL display a persistent, visible stale-client notice instead of reloading again. Any native client of the API SHALL perform the same comparison and surface a visible notice on mismatch; clients MUST NOT silently continue against a server whose `apiRevision` differs from their own.
...
- **WHEN** a client's embedded `apiRevision` differs from the server's
- **THEN** the client surfaces the mismatch visibly rather than silently continuing
