...
Define how uatu bundles a full Nerd Font asset (Hack Nerd Font Mono) into its compiled binary, exposes it through a stable HTTP route, registers it via `@font-face` with local-then-url precedence, and tracks the upstream licenses so `bun run check:licenses` keeps passing. This guarantees that ASCII and Nerd Font icon glyphs render correctly out of the box in every browser — including ones (e.g., Safari) that do not expose user-installed system fonts to web pages — without forcing end users to install fonts themselves.
