...
Define the Review-mode Selection Inspector pane that captures Source-view text selections as Claude-Code-style `@path#L…` references, hints when Rendered view cannot capture line ranges, and provides a clipboard-copy affordance for the captured reference.
