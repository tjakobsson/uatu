...
Define a single, app-wide monospace surface story for uatu: one CSS custom property (`--mono-font-family`) that governs every monospace surface in the UI, an optional `.uatu.json mono.fontFamily` override that flows from the server through state into that variable, and a narrower `terminal.fontFamily` override that wins inside the terminal panel without affecting the rest of the UI. Vendor stylesheets (notably `github-markdown-css`) are overridden via specificity, not `!important`.
