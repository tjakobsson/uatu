...
- [x] 8.17 Make a failed inventory read distinguishable from an empty one (`null`, not `[]`) and move the sweep decision into a pure `parkedPanesToSweep`. A blip previously read as "every parked session was terminated" and deleted panes whose PTYs were alive in another window — taking the last one closed the panel and dropped its record. Pinned by unit test, not E2E: uatu's pass-through service worker means Playwright's `page.route` cannot fail that request, and the aborted-request version of the test silently passed against the bug
- [x] 8.18 Hand the active slot to a surviving pane in `handlePaneUnavailable` instead of nulling it — touch mode shows only the pane carrying `data-active`, so a restore that collided on the active record blanked the Terminal tab while live terminals sat hidden behind it
