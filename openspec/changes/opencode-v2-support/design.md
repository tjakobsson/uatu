...
- `session.renamed` carries only `sessionID` and `title`. It is normalized as a sparse lifecycle update (`sparse: true`), and the adapter keeps the parent it already knows (or reads it) instead of clearing it; a full `session.updated` record without a parent still means top-level, as on 1.x.
