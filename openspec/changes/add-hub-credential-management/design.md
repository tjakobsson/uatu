...
Runtime sockets, generated Git/SSH configuration, provider CLI views, and process ownership records live under a mode-0700 `credential-runtime/` directory and are never treated as durable state. A process-lifetime state-root lease prevents two live Hubs from mutating one catalog or dedicated GnuPG home. The lease is a permanent owner-only SQLite file at `.hub-lease`; it is created once without following symlinks and is never renamed, replaced, or removed. After validating the file type, owner, exact mode, link count, device, and inode, the Hub opens it with SQLite no-follow and extended-error flags, requires the existing DELETE journal mode and EXCLUSIVE locking mode, initializes its tiny schema, and retains one raw `BEGIN EXCLUSIVE` transaction for the Hub lifetime. Only SQLite busy errors mean another Hub owns the lease. Corrupt databases, unsafe paths, unsupported locking, and all other I/O failures stop startup. A legacy lease directory also stops startup with manual stop-and-remove guidance. Graceful release rolls back and closes the connection; process death releases the kernel lock. Operators must keep the state root on a local filesystem with reliable SQLite/VFS locking and must not rename or replace the permanent lease inode while the Hub runs. SQLite cannot protect the pathname from a malicious process running as the same UID, which is already outside the local backend's stated protection boundary. Startup preserves existing runtime files until their ownership can be checked, and it never removes another live Hub's sockets or session configuration. Atomic state writers apply owner-only permissions before the destination rename so a reported write failure cannot leave disk and memory on different revisions. Read APIs build explicit public DTOs rather than serializing persisted records. Secret mutation handlers use dedicated request shapes and must never include request bodies in errors or logs.
...
Before exposing credential readiness, Hub startup constructs a pathless recovery manager and retires any persisted SSH guardian. Recovery therefore runs even when current tool discovery cannot supply `ssh-agent`, clears every identity retained across the restart, and completes before a later graceful shutdown can omit the old guardian. Fresh-agent startup remains lazy. When first needed, the Hub starts a detached internal guardian through the same uatu binary. The guardian starts `ssh-agent` as its direct child in foreground mode with a fixed socket below `credential-runtime/` and retains the Bun subprocess handle for the child's full lifetime. Startup configuration crosses a private inherited pipe and uses a bounded ready/commit/ack exchange: the Hub durably writes a mode-0600 version-2 ownership record only after the guardian reports owner-only agent and control socket identities, and the guardian accepts the commit only while the child remains live and both socket identities still match. If commit is absent or invalid, the guardian stops its direct child and cleans the uncommitted artifacts. After commit it survives Hub stdin closure and parent death.

The ownership record contains a random nonce, diagnostic-only PIDs, and the UID, mode, device, and inode of both Unix sockets. The Hub never reconstructs or signals a recorded PID. Recovery and graceful shutdown send `status` or `stop` with a fresh challenge over the private control socket; the guardian returns HMAC-SHA256 over the canonical command and challenge without disclosing the nonce in the request. Only the guardian calls `kill()` on its retained child handle. After requested or unexpected child exit it closes the listener, atomically renames each exact owned socket to a nonce-derived quarantine path, verifies identity before unlinking, then similarly quarantines and verifies the exact committed ownership record. A clean child exit may already have removed its agent socket; the retained exit result makes that specific absence safe while all other missing or replaced artifacts fail closed. A mismatch is restored when possible or preserved in quarantine. The Hub serializes start, recovery, and shutdown, validates cached state with authenticated status, waits for all three public paths and all three nonce-derived quarantine paths to disappear after stop, and starts fresh only after guardian cleanup. Startup scans for known quarantine names even when the public record is absent, so interrupted cleanup requires explicit repair rather than being mistaken for success. Old records, missing or replaced sockets, failed authentication, and unresponsive guardians fail closed without cleanup or replacement spawning.

All unlocked Hub SSH identities may reside in that agent; only an `ssh-agent` override change restarts it — key-tool override changes rebuild the credential service around the running agent so loaded identities and explicit locks survive. Workspace assignment selects a public identity through generated SSH/Git configuration (`IdentityFile` referencing public material plus `IdentitiesOnly`, and Git's SSH signing-key configuration) and invokes the validated absolute `ssh` client path; it does not claim that another same-UID process cannot query the shared agent.
...
SSH generation and encrypted-key import run in a private bounded PTY so `ssh-keygen` can read the key's passphrase without placing it in process arguments; terminal echo is disabled before the secret is written and all captured output is sanitized. Unlock runs detached from the Hub's controlling terminal and supplies `ssh-add` through forced `SSH_ASKPASS` over an owner-only named pipe in `credential-runtime/`, resolved through the service `PATH`. FIFO setup and secret subprocesses terminate their process groups on timeout so descendants cannot retain channels or recreate cleaned artifacts. Import keeps the private key's existing encryption rather than changing its passphrase. Failed import revokes any identity that may have reached the agent before catalog and backing cleanup. Unencrypted imports use bounded non-interactive public-key derivation and are automatically loaded whenever the managed agent is recreated. Agent contents are runtime state, so protected SSH credentials return locked after Hub restart.

OpenPGP unlock performs a bounded signing probe through the dedicated GnuPG home using loopback secret input, allowing `gpg-agent` to establish its normal cache. The secret is discarded immediately. Generation inventories secret keys before and after the command, so a key created before timeout, output overflow, or unsuccessful exit is removed even when status output is incomplete; ambiguous inventory or cleanup failures are reported instead of leaving an apparently successful operation. A locked credential used by a workspace fails with an actionable message; arbitrary Git processes do not trigger a browser modal. Because the GnuPG home has one shared agent and no configured per-key cache-eviction tool, the UI does not expose a credential-specific OpenPGP Lock action. Disabling blocks new Hub use without killing the shared agent or locking unrelated credentials; whole-agent termination is reserved for Hub shutdown. This avoids a general cross-process pinentry routing service and makes user intent explicit.
...
The generated Git command-scope configuration takes precedence over repository/global defaults for managed fields while leaving unrelated user Git configuration available. At session start the local backend flattens effective system/global Git settings into an owner-only generated file, omits credential-, HTTP-, URL-, remote-, password-, proxy-, and certificate-bearing entries, and runs ordinary Git with an isolated home so `.netrc` cannot supply ambient authentication. Repository configuration retains its normal precedence, and the workspace shell keeps its normal `HOME`. Generated helper commands and SSH configuration quote both shell and tool-level syntax. Assignment hosts are restricted to literal DNS names or IP addresses so a generated Host block can never contain OpenSSH pattern metacharacters; runtime host matching canonicalizes equivalent DNS and IPv6 spellings. A broad host block excludes every port another assignment claims for the same hostname, keeping match blocks mutually exclusive despite OpenSSH's additive IdentityFile semantics. The local backend places generated Git and `ssh` shims first on `PATH` for ordinary workspace use and gives unassigned workspaces an empty GnuPG home; absolute tool paths and environment changes remain within the documented same-UID bypass. Each session generation uses a distinct runtime directory, creation never removes another generation, and cleanup removes only the owning session's files. The exact environment/config representation stays internal to the backend. A local process can unset it, inspect same-UID runtime files, or reach the shared agent; the UI and API warning describe that limitation.
...
Clone creation accepts an optional credential id and retain-assignment choice. The server validates that the credential type matches the remote transport and host before creating the job. Managed SSH selection configures the validated absolute SSH client, public identity, and managed socket; the clone re-resolves its selection inside the credential runtime section and holds that section for its process lifetime so an ssh-agent override defers until it exits; managed HTTPS selection configures only the Hub helper. Every clone disables global and system Git configuration because arbitrary `url.*.insteadOf`, HTTP authorization, cookie, proxy, and TLS client settings cannot be reset safely with a finite command-scope list. It also isolates SSH from the Hub user's configuration and default identities (-F /dev/null plus IdentityFile none). No selection removes inherited agent/helper variables and retains today's PTY response flow, so an unselected SSH remote authenticates only interactively.

After clone success, registration and assignment persistence are coordinated: a requested retained assignment is written only for the successfully registered workspace. API assignment, clone assignment writes, clone rollback, and workspace forget all use the existing per-workspace session lifecycle queue, and Forget refuses a workspace whose clone target remains reserved. An assignment therefore cannot commit after forget or rollback cleanup. The confirmed stop-and-remove action performs its unassignment inside the stop lifecycle operation so a concurrent start cannot land between the two state changes. If session startup then fails and registration rolls back under the existing contract, the assignment rolls back too. If registration removal fails, the Hub restores the retained assignment so the still-registered workspace does not silently lose it. Interactive clone responses remain ephemeral, exact submitted values and partial secret prefixes are filtered from live and retained output, and responses are never offered as saved credentials implicitly.

### D8: Tool configuration is persisted state with layered probes

Each tool has a default executable name searched through the Hub service's `PATH` and an optional dashboard-managed absolute override. Overrides are accepted only for executable regular files and are stored after validation. A persisted override that later becomes invalid degrades to unavailable readiness so Settings remains reachable for repair; it does not fall back to a different ambient executable. Override persistence, re-probing, and runtime replacement form one compensating transaction, so a failed runtime application restores the prior override and runtime or reports restoration failure. Every subprocess probe has fixed argv, timeout, output cap, descendant termination, shutdown tracking, and sanitized error mapping.
...
Credential listing, tooling readiness, and public-key export are authenticated. Every mutation uses POST plus the existing same-origin check. Secret inputs use no-store responses on success and on pre-dispatch authentication or origin failure, and are excluded from retained job/event models. Request parsers reject fields excluded by the closed public schemas. Rate limits bound passphrase attempts and expensive key generation. Readiness failures degrade one credential instead of hiding the inventory. Test output is structured, capped, and mapped to known stages rather than returning raw stderr.

The server-rendered Hub keeps sessions and workspaces on `/`, puts credential management, tool readiness, and devices on `/settings`, and puts folder registration and cloning on `/clone`. Shared navigation links all three pages. Forms are capability-specific instead of one polymorphic secret form, and private-key paste controls are masked until explicitly revealed. Destructive operations name the credential and require confirmation; deleting an assigned credential either fails with references or performs an explicit delete-and-unassign transaction. Paired authentication/signing changes use one server-side metadata transaction under the workspace queue and both credential locks rather than browser-side compensation. The clone page fetches public credential DTOs directly and does not depend on settings-page DOM or initialization.
...
Disable, lock, unassign, and delete update the catalog first, then remove cached agent identities or generated runtime references. Disable, lock, and delete additionally run under every assigned workspace's session lifecycle queue, so a starting session cannot capture a credential the API is reporting revoked; assignments commit under a per-credential lock the revocation re-snapshots against, so an assignment in flight for an unlisted workspace widens the revocation's lock set instead of escaping it. An explicit SSH lock is remembered for the Hub's lifetime: readiness-triggered auto-load rechecks that state inside the per-credential agent-mutation chain and cannot clear it, while an explicit unlock or re-enable may do so. Agent-mutating SSH operations serialize per credential, so a pending unlock cannot land its identity after a concurrent revocation finished removing the credential. The helper and all new session/clone resolution consult current catalog state, so new operations fail immediately. Existing SSH multiplexed connections or provider-issued sessions cannot reliably be revoked by deleting a local key; the dashboard states this limitation.
...
- **[State root uses unreliable network-filesystem locks]** → Operators must place Hub state on a local filesystem with working SQLite/VFS locks. Runtime checks cannot prove that a network filesystem implements locks reliably.
