...
Clone creation accepts an optional credential id and retain-assignment choice. The server validates that the credential type matches the remote transport and host before creating the job. Managed SSH selection configures the validated absolute SSH client, public identity, and managed socket, and the clone holds the credential runtime section for its process lifetime so an ssh-agent override defers until it exits; managed HTTPS selection configures only the Hub helper. Every clone isolates SSH from the Hub user's configuration and default identities (-F /dev/null plus IdentityFile none); no selection removes inherited agent/helper variables and retains today's PTY response flow, so an unselected SSH remote authenticates only interactively.
