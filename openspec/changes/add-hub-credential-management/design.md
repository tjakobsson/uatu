...
Clone creation accepts an optional credential id and retain-assignment choice. The server validates that the credential type matches the remote transport and host before creating the job. Managed SSH selection configures the validated absolute SSH client, public identity, and managed socket; the clone re-resolves its selection inside the credential runtime section and holds that section for its process lifetime so an ssh-agent override defers until it exits; managed HTTPS selection configures only the Hub helper. Every clone isolates SSH from the Hub user's configuration and default identities (-F /dev/null plus IdentityFile none); no selection removes inherited agent/helper variables and retains today's PTY response flow, so an unselected SSH remote authenticates only interactively.

After clone success, registration and assignment persistence are coordinated: a requested retained assignment is written only for the successfully registered workspace. API assignment, clone assignment writes, clone rollback, and workspace forget all use the existing per-workspace session lifecycle queue, so an assignment cannot commit after forget or rollback cleanup, and the confirmed stop-and-remove action performs its unassignment inside the stop lifecycle operation so a concurrent start cannot land between the two state changes. If session startup then fails and registration rolls back under the existing contract, the assignment rolls back too. If registration removal fails, the Hub restores the retained assignment so the still-registered workspace does not silently lose it. Interactive clone responses remain ephemeral and are never offered as a saved credential implicitly.
