...
Runtime sockets, generated Git/SSH configuration, provider CLI views, and process ownership records live under a mode-0700 `credential-runtime/` directory and are never treated as durable state. A process-lifetime state-root lease prevents two live Hubs from mutating one catalog or dedicated GnuPG home; stale-lease recovery uses an exclusive reclaim claim and verifies ownership again before removal. Startup preserves existing runtime files until their ownership can be checked, and it never removes another live Hub's sockets or session configuration. Atomic state writers apply owner-only permissions before the destination rename so a reported write failure cannot leave disk and memory on different revisions. Read APIs build explicit public DTOs rather than serializing persisted records. Secret mutation handlers use dedicated request shapes and must never include request bodies in errors or logs.
