...
After clone success, registration and assignment persistence are coordinated: a requested retained assignment is written only for the successfully registered workspace. API assignment, clone assignment writes, clone rollback, and workspace forget all use the existing per-workspace session lifecycle queue, so an assignment cannot commit after forget or rollback cleanup. If session startup then fails and registration rolls back under the existing contract, the assignment rolls back too. If registration removal fails, the Hub restores the retained assignment so the still-registered workspace does not silently lose it. Interactive clone responses remain ephemeral and are never offered as a saved credential implicitly.
