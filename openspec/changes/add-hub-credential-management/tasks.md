...
- [x] 2.2 Implement layered version/runtime/readiness probes with timeouts, output caps, sanitized structured results, tool-identity banners so unrelated executables are rejected, and independent degradation per capability.
...
- [x] 3.1 Implement a detached guardian-owned `ssh-agent` using fixed owner-only agent/control sockets, bounded pipe startup with durable commit/ack, versioned socket-identity ownership, authenticated status/stop control, serialized proactive recovery plus lazy fresh startup and shutdown, guardian-only quarantine cleanup after requested or unexpected exit, stale-state recovery without numeric PID signals, and bounded fail-closed behavior.
...
- [x] 14.8 Address post-sweep Codex findings by sanitizing effective workspace system/global Git configuration and `.netrc` use while preserving unrelated settings, and reconciling OpenPGP generation against secret-key inventory after timeout or unsuccessful completion.
- [x] 14.9 Harden SSH guardian recovery after review: make committed cleanup survive Hub death, proactively recover before readiness even without current SSH tooling, verify the direct child and sockets at commit, replace nonce-bearing requests with HMAC challenge-response, serialize cached lifecycle checks, close timed-out control clients, reject trailing startup input, tolerate the owned socket removal of a clean child exit, fail closed on interrupted nonce quarantine, avoid unchanged-path agent restarts during forced tool rollback, and cover forged responses plus parent SIGKILL recovery.
- [x] 14.10 Replace the state-root lease with a permanent owner-only SQLite file held by a process-lifetime exclusive transaction, fail closed on unsafe or legacy paths and non-busy SQLite errors, and cover real subprocess contention, crash release, stable inode, fresh DELETE-mode sidecar absence, retained-lock shutdown, and compiled/interpreted locking.
- [x] 14.11 Track each running session's actual credential projection; acquire revocation queues from assignments plus live projections; stop projected provider CLI token users before disable or confirmed deletion; preserve the catalog on stop failure; and cover stale replacement, post-start assignment, HTTPS-only, rejected, concurrent, aggregate-failure, and unrelated-workspace cases.
