...
let html = '<!doctype html><meta charset="utf-8"><title>Corrected mobile Hub chrome review</title><style>body{font:16px system-ui;margin:24px;background:#eee}img{max-width:100%;height:auto;width:390px}section{margin:40px 0}a{margin-right:16px}h2{margin-bottom:8px}.versions{display:flex;gap:12px;flex-wrap:wrap}</style><h1>Corrected chrome — awaiting user review, not approved goldens</h1><p>Retained actual synthetic frontend captures with canonical original reference names. Earlier derived PNGs are recoverable at their original paths in pre-cleanup Git commit 8fc4811. No masks. Differences in branding, fixture text and current workspace interiors are not failures. Right-side adaptations compare to the retained state rather than silently mirroring originals. Read <a href="README.md">README.md</a> for historical findings and limits. <a href="../index.html">Initial discovery gallery</a></p>';
...
for (const browser of ['chromium', 'webkit']) {
  ...
  for (const [state, value] of Object.entries(data) as Array<[string, any]>) {
    ...
    html += `<section><h2>${browser}: ${state}</h2><p>Canonical original reference: ${value.comparison.reference}. <a href="${browser}-${state}-actual.png">Actual</a></p><img src="${browser}-${state}-actual.png" alt="${browser}: ${state}"></section>`;
