...
**Reason**: Restated as "Diff view renders the active file's git diff against the resolved compare base": the resolution priority no longer includes a configured `review.baseRef` (the `.uatu.json` `review` block is removed), and review-base phrasing is retired.
**Migration**: None — Diff view behavior is unchanged apart from configured-base support; repositories that relied on `review.baseRef` fall back to automatic base resolution.
