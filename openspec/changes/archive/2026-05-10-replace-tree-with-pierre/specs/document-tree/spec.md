...
**Reason**: Replaced by the new requirement "Use uatu's file-type icon registry for tree rows", which preserves the same icon-registry contract but wires it through `@pierre/trees`' icon configuration API and drops the obsolete "non-clickable" qualifier on binary rows (binary entries are now clickable and route to the existing preview-unavailable view).
**Migration**: Existing icon registry entries in `src/file-icons.ts` are reused unchanged. No author-facing migration required.

### Requirement: Show last-modified time on each tree row
**Reason**: The 1-second-ticking relative-time label per row (file leaves and directory rollup) is deferred. It is uatu-distinctive and will return as a separate change once the library's row-annotation surface is understood — likely as a row annotation or a "recently active" pulse rather than a per-row text label.
**Migration**: No author-facing migration. The visual mtime cue is gone for now; users can still observe file-change activity through the live-update channel and the upcoming git-status annotations on changed rows.

### Requirement: Render directory rows in the file tree with a folder icon
**Reason**: `@pierre/trees` renders folder rows with its own folder icon as part of its default DOM. uatu no longer owns directory-row rendering, so the bespoke folder-icon requirement is obsolete.
**Migration**: None. Directory rows still display a folder icon, sourced from the library.
