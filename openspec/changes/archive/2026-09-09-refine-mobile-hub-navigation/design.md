...
- No new dependency, font, manual version bump, or production import of prototype code, assets, fixtures, styles, or preference keys. The approved prerequisite exception below permits minimal root-qualified image delivery; any public-contract addition must follow API documentation and validation rules.
...
### Approved Prerequisite Corrections

After the initial implementation exposed existing owner limitations, the user
explicitly approved expanding this change to correct them before completing the
UI. These are narrow correctness prerequisites, not a broader backend redesign:

- Carry stable root/document identity through image resource resolution and
  Back/Forward restoration. Preserve ordinary binary links, containment/ignore
  checks, effective scope, and existing URL compatibility. Prefer existing
  delivery paths; document and validate any additive public resource contract.
- Recover stopped or removed workspace history targets even when Back interrupted
  initial bundle loading. A minimal document-cache policy correction or early
  document recovery hook is allowed. It must not issue Start, substitute another
  workspace, introduce a resident host, or change authentication/session lifetime.
- Replace unguarded Terminal storage fallbacks with safe client-local fallback
  behavior when storage access itself throws. Preserve workspace-scoped keys and
  avoid any change to PTY creation, attachment, takeover, or credential policy.

These corrections are covered by tasks 3.3, 5.1, and 6.2 and their regression
tests. The original no-server-change constraint is relaxed only for this work;
the no-session-lifecycle and no-prototype-import constraints remain in force.
