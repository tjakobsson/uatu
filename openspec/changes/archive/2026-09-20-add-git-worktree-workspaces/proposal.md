...
The published contract is Hub revision 8 / workspace revision 20 (OpenAPI `8.20.0-experimental`). This user-approved decision follows the 2026-09-19 rebase onto `origin/main` at `a1ac6d3`: notification `allWorkspaces` already owns Hub revision 7, and worktree additions to closed state objects and the live topic require a subsequent Hub revision under the compatibility policy, even while both are unreleased. Workspace payloads retain revision 20; upstream notification and receipt changelog history remains separate and intact.
