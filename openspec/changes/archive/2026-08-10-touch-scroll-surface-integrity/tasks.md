...
- [x] 7.7 File any blast-radius misbehaviour the pass surfaces that is not fixed here, rather than widening this change — nothing to file: the maintainer's iPhone and iPad pass found no misbehaviour beyond what this change fixes, including the five blast-radius call sites (find-bar navigation, the outline scroll spy, the two scroll-to-top paths, and in-page anchor clicks) that were read statically rather than device-verified when the change was written
