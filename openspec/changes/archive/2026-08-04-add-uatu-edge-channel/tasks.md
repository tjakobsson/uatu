...
- [x] 2.1 Refactor `scripts/generate-formula.ts` to mirror `generate-cask.ts`: export `generateFormula(version, sums, {name, tag})` (reusing/sharing `parseSums`), keep the CLI wrapper, add `--name` / `--tag` flags; derive the Ruby class name from the formula name (`uatu` → `Uatu`, `uatu@edge` → `UatuATEdge`)
