...
- [ ] 2.1 Compute the covered-chrome height from the window's
...
- [ ] 2.2 Observe `contentLayoutRect` changes (native tab bar appear/
...
- [ ] 2.3 Apply the same resolved inset as top padding to the split-browser
      pane's tab strip and to the launcher/starting/failure layouts

## 3. Inset consumption (SPA)

- [ ] 3.1 Add `html.uatu-desktop-host` CSS: top padding/offsets on the
...
- [ ] 3.2 Verify the terminal panel and any other full-height chrome
      surfaces against the inset (adjust only if they reach the top strip)
- [ ] 3.3 Confirm the no-marker path is byte-identical layout for browser
      and PWA

## 4. Verification

- [ ] 4.1 E2E test for the SPA contract: inject the marker class + variable,
...
- [ ] 4.3 Run `bun test`, `bun test:e2e`, and desktop CI's build path
