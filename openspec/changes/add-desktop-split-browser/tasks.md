...
- [ ] 1.1 Add a `BrowserSplitState` model per window (open/closed, width,
      `[BrowserTab]`, selected index) and a persistent shared
      `WKWebsiteDataStore` (identifier-based, separate from the uatu
      WebView's store)
- [ ] 1.2 Wrap the existing hosting view in a resizable split layout;
      right pane renders the browser surface when open, uatu pane fills
      the window when closed
- [ ] 1.3 Add "Toggle Split Browser" (`⌘⇧B`) to `UatuCodeDesktopCommands`,
      disabled without a running server

## 2. Tabs and chrome

- [ ] 2.1 Build the custom tab strip (title, close button, overflow
      scrolling); keep unselected tabs' pages alive; closing the last tab
      closes the split
- [ ] 2.2 Build the per-tab chrome row: back/forward/reload wired to the
      tab's page, plus the eject (⧉) control (open in system browser +
      close tab)
- [ ] 2.3 Implement the editable address bar: URL/bare-host input loads
      (prefix `https://`), other input searches via the default engine
      chosen in design D4
- [ ] 2.4 Route `⌘W` / `⌘[` / `⌘]` by focused surface (browser tab vs uatu
      pane / native window tab)

## 3. Link routing

- [ ] 3.1 Add the `AppStorage` opt-out setting ("Open external links in
      system browser") as a menu toggle
- [ ] 3.2 Redirect the `openExternally` `http(s)` branch from
...
- [ ] 3.3 Detect `⌘`-click at the interception point so it bypasses the
...
- [ ] 4.3 Build desktop app locally (`bun run build` + Xcode build) and
