...
- [x] 1.1 Archive `add-opencode-chat` so the `opencode-chat` spec exists under
  `openspec/specs/` (this change's delta is written against that result)

## 2. Layout — DOM and CSS

- [x] 2.1 Wrap `.preview-shell` + `#chat-surface` in a `.work-row` flex row
...
- [x] 2.2 Add the chat split divider element and the collapsed-state slim
  strip with its reopen toggle (accessible name, keyboard operable)
- [x] 2.3 Remove the two `main-surface-switch` markup instances, their styles,
...
- [x] 2.4 CSS for the split: fractional widths via a custom property, minimum
...
- [x] 2.5 Replace the inline pre-paint boot script's main-surface handling
  with panel-state equivalents (collapsed attribute + width variable) so
  first paint doesn't flash

## 3. Panel state module

- [x] 3.1 Rewrite `src/chat/surface.ts` as chat-panel state: open/collapsed +
  ...
  `src/shell/state.ts`
- [x] 3.2 Implement divider drag (pointer capture, clamps, fraction commit)
  and the collapse/reopen toggle wiring
- [x] 3.3 Implement the narrow-viewport guard: auto-collapse below the
...
- [x] 3.4 Unit tests for persistence round-trip, clamping, guard
  yield-and-restore, and reopen-restores-fraction

## 4. Call-site simplifications

- [x] 4.1 `src/find/active-surface.ts`: delete the `onMainSurfaceChange`
...
- [x] 4.2 `src/chat/file-references.ts`: drop `setMainSurface("preview")` —
  desktop navigates in place, touch still switches to the Preview tab
- [x] 4.3 `src/find/find-bar.ts`: reveal-chat-for-find becomes
  expand-if-collapsed via the panel API
- [x] 4.4 `src/shell/tab-bar.ts`: replace the tab → mainSurface sync with
...
- [x] 5.1 Run `bun run dev` and visually verify all four states before any
...
- [x] 5.2 Add a shared e2e helper that activates Chat via the panel
  ...
  exclusivity
- [x] 5.3 New e2e coverage for the split: fraction persistence across reload,
  ...
  collapsed panel
- [x] 5.4 Full `bun test` and `bun test:e2e` green; `openspec validate
