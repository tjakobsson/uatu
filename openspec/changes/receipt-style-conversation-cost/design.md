...
Within a child session, assistant messages are grouped by the user message they answer (`parentID`); the child's user messages in order correspond to the launching conversation's `task` rows for that child in order. The k-th row's usage is the sum over the k-th group.
...
- Groups with no matching row — a prompt typed into the child from the OpenCode TUI, a compaction inside the child — are attributed to the latest preceding row, so nothing is dropped from the total. A child with rows but no matching group yet (task just launched) has no usage, as today.
...
- [Prompt↔row pairing drifts — a child prompted outside `task`, or a task row whose launch failed before its prompt was stored] → pair by order only among rows that named the child, attribute unmatched groups to the preceding row, and test that the rows always sum to the session.
