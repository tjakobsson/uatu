...
**Reason**: Mandatory per-session worker registration conflicts with manifest-only installation and the hub-owned push worker shared across workspaces.
**Migration**: Replace this requirement with "PWA and push worker scopes follow serving ownership". Generic mounts retain path-scoped manifests without a worker; hub pages share the origin-scoped push registration after identified legacy registrations are cleaned up.
