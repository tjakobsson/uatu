...
export const optionalEvidenceSelection = Object.freeze([
  ...
  "navigation-recovery/verification.md", "navigation-recovery/browser-results.json", "navigation-recovery/boundary-results.json", "navigation-recovery/production-results.json",
  ...["chromium", "webkit"].flatMap(engine => [
    ...[320, 390, 844].map(width => `navigation-recovery/${engine}-creation-cancel-${width}.png`),
    ...["no-subfolders", "security", "devices", "preview-resumed", "preview-map.svg", "preview-sample.bin", "preview-index-error", "preview-index-recovered"].map(scene => `navigation-recovery/${engine}-${scene}.png`),
    ...["normal", "empty", "error", ...[320, 390, 844].flatMap(width => [`${width}-normal`, `${width}-scrolled`])].map(scene => `navigation-recovery/folder-picker/${engine}-${scene}.png`),
    ...["guide-image", "asciidoc-image", "asciidoc-source", "nested-files-tree", "mermaid-viewer", "asciidoc-mermaid", "notes-direct-reload", "notes-linked-runbook"].map(scene => `navigation-recovery/previews/${engine}-preview-examples-${scene}.png`),
  ]),
  ...["chromium", "webkit"].flatMap(engine => [
