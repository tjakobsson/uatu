import { action, check, choiceGroup, destinationRow, field, group, infoRows, listRow, select, text } from "../../src/hub/mobile/design-system";
...
${example("navigation", group("Navigation and actions", destinationRow("local-destination", "Credential details", "shield", "Opens a read-only page", "SSH", "green") + listRow("local-list", "Example workspace", "Object navigation, not a start command", "folder") + action("local-action", "Run local action") + action("disabled-action", "Unavailable action") + action("confirmation", "Remove example…", true)), "Chevrons navigate to full-page details. Immediate actions update local status; destructive actions name consequences and request explicit confirmation. Selection is not a primary action.")}
