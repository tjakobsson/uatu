// Failure diagnostics for tests that launch their own browser.
//
// A spec that calls `chromium.launch()` / `webkit.launch()` itself runs
// outside the configured `page` fixture, so a Playwright trace of it records
// only the API calls — none of the page's console, errors, or network. When
// such a test fails once in a hundred runs (a WebKit page that never left
// "Connecting", say), there is nothing left to explain it. Global tracing is
// not the answer: it slowed the performance-budget suites past the CI time
// limit. Instead, `launchBrowser` records a cheap text log for every context
// the browser opens — console messages, uncaught page errors, each non-asset
...
import { chromium, test as base, webkit, type Browser, type BrowserContext, type LaunchOptions, type Page, type TestInfo, type TestType } from "@playwright/test";
...
/** Launches `engine` for the current test with every context it opens
 *  recording page diagnostics (see the header). Drop-in for
 *  `({ chromium, webkit })[engine].launch(options)`. */
export async function launchBrowser(engine: Engine, options?: LaunchOptions): Promise<Browser> {
  const testInfo = base.info();
  const log = logFor(testInfo);
  const browser = await ({ chromium, webkit })[engine].launch(options);
  push(log, `[${engine}]`, `launched ${browser.version()}`);
  // Both ways a test opens a page go through a context this module watches
  // from before the first navigation. (Playwright's own `newPage` opens its
  // context through `newContext`, so a context is only instrumented once.)
  const instrumented = new WeakSet<BrowserContext>();
  let contexts = 0;
  const instrument = async (context: BrowserContext) => {
    if (instrumented.has(context)) return;
