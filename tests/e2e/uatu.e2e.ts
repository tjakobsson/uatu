...
test("Source / Rendered toggle is hidden for source files and visible for Markdown / AsciiDoc", async ({ page }) => {
...
test("Layout chooser is visible for Markdown / AsciiDoc and hidden for source files", async ({ page }) => {
  ...
  await expect(page.locator("#layout-chooser")).toBeVisible();

  // Switch to a code/text file: both controls hide.
  await fs.writeFile(workspacePath("split-chooser.json"), "{\"key\": \"value\"}\n", "utf8");
  const codeFile = treeRow(page, "split-chooser.json");
  await expect(codeFile).toBeVisible();
  await codeFile.click();
  await expect(page.locator("#view-control")).toBeHidden();
  await expect(page.locator("#layout-chooser")).toBeHidden();
...
test("Side-by-side layout renders Source left, Rendered right, with both visible", async ({ page }) => {
  ...
  await page.locator("#layout-split-h").click();
  await expect(page.locator("#layout-split-h")).toHaveAttribute("aria-checked", "true");
...
test("Stacked layout renders Source on top, Rendered below", async ({ page }) => {
  ...
  await expect(page.locator("#layout-split-v")).toHaveAttribute("aria-checked", "true");
...
test("Layout preference persists across reload and across documents", async ({ page }) => {
  // Default is single.
  await expect(page.locator("#layout-single")).toHaveAttribute("aria-checked", "true");
...
test("Switching to single layout preserves the Source / Rendered preference", async ({ page }) => {
  ...
  // Enter side-by-side; the toggle hides but the preference is preserved.
  await page.locator("#layout-split-h").click();
  await expect(page.locator("#view-control")).toBeHidden();
