...
describe("ClaudeProvider sessions", () => {
  ...
  test("an elicitation form becomes question steps and answers as coerced MCP content", async () => {
    ...
    await expect(provider.replyQuestion(session.id, "el-multi", [["Repositories", "Gists"], ["Pro"], ["1.5"]])).rejects.toThrow(/whole number/);
    await expect(provider.replyQuestion(session.id, "el-multi", [["Repositories"], ["Pro"], ["9"]])).rejects.toThrow(/at most 5/);
    await expect(provider.replyQuestion(session.id, "el-multi", [["Repositories"], ["Pro"], ["x"]])).rejects.toThrow(/enter a number/);
    expect(await provider.listQuestions!()).toEqual([expect.objectContaining({ requestId: "el-multi" })]);
    await provider.replyQuestion(session.id, "el-multi", [["Repositories", "Gists"], ["Pro"], [" 3 "]]);
    expect(await multi).toEqual({ action: "accept", content: { scopes: ["repo", "gist"], tier: "pro", count: 3 } });
