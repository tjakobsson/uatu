...
export function readiness(rows: readonly ReadinessResult[], prerequisite = ""): string {
  ...
  return `<div class="mh-readiness"><div class="mh-readiness-summary">${infoRows([
    { label: "Result", value: summary.label, tone: prerequisite || blockers.length ? "warning" : "neutral" },
    { label: "Scope", value: "Local setup only", detail: "Remote permissions have not been checked." },
    ...(prerequisite ? [{ label: "Required action", value: prerequisite, tone: "warning" as const }] : []),
  ])}</div>${blockers.length ? `<h3>Needs attention</h3><ul class="mh-readiness-blockers">${[...new Set(blockers.map(r => r.message))].map(message => `<li>${esc(message)}</li>`).join("")}</ul>` : ""}</div>`;
}
...
export function diagnosticReport(rows: readonly ReadinessResult[]): string {
  ...
  const row = (r: ReadinessResult) => `<div data-readiness-layer="${esc(r.layer)}" data-readiness-status="${esc(r.status)}">${infoRows([{ label: "Result", value: statuses[r.status], detail: r.message, tone: r.status === "unavailable" ? "warning" : "neutral" }])}</div>`;
