...
export function worktreePage(m: WorktreePresentation, reviewChrome = "", reviewScript = "", fragment = false): string {
  ...
  if (m.view === "inventory") {
    ...
    body = `<section class="wt-creation" data-creation><h2 class="wt-heading" tabindex="-1">${existing ? "Existing branch" : "New branch / worktree"}</h2>${existing ? `<form id="wt-fetch" method="post" action="${h(m.prefix)}/fetch" data-operation data-pending="Fetching remote branches…">${hidden("source", m.source.id)}</form>` : ""}${post("create", `${hidden("mode", existing ? "existing" : "new")}${existing ? `${hidden("selection", draft.selection ?? "")}<div style="display:flex;gap:8px;align-items:end"><label style="flex:1;min-width:0">Branch<input name="query" value="${h(draft.query ?? "")}" data-branch-search role="combobox" aria-autocomplete="list" aria-expanded="true" aria-controls="wt-branch-list" placeholder="Search branches…" autocomplete="off"></label><button form="wt-fetch" aria-label="Fetch remote branches" title="Fetch remote branches" style="padding:8px"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" aria-hidden="true"><path d="M20 7v5h-5M4 17v-5h5M6 7a7 7 0 0 1 12-1l2 6M4 12l2 6a7 7 0 0 0 12-1"/></svg></button></div><div id="wt-branch-list" class="wt-branches" role="listbox" aria-label="Branches">${branches.map(({ ref, kind }, index) => `<div id="wt-branch-${index}" class="wt-branch" role="option" aria-selected="${draft.selection === `${kind}:${ref}`}" data-value="${h(`${kind}:${ref}`)}" data-search="${h(ref)}"><span>${h(ref)}</span><small>${kind === "remote" ? "Remote" : "Local"}</small></div>`).join("")}<p data-branch-empty class="wt-muted" role="status" hidden style="padding:12px">No matching branches</p></div>` : input("branch", "Name", "")}<div class="wt-actions"><button type="button" data-cancel>Cancel</button><button class="primary" data-create${existing && !draft.selection ? " disabled" : ""}>Create</button></div>`, "Creating…")}</section>`;
    ...
    if (m.view === "delete") {
      ...
      body = `<section data-compact><h2 class="wt-heading" tabindex="-1">Delete worktree?</h2>${identity}${blocked ? `${m.error ? `<p role="alert" tabindex="-1">${h(m.message ?? "Deletion could not be completed. Files and registration retained.")}</p>` : '<p role="alert" tabindex="-1">Only a verified Uatu-created checkout can be deleted. Restore or verify it outside Uatu.</p>'}<div class="wt-actions" style="justify-content:flex-end">${cancel}</div>` : post("delete", `${hidden("id", row.id)}${hidden("confirm", "1")}<p>${row.running ? "Its Uatu terminal and agent sessions will stop, then the worktree’s files will be removed. The Git branch will be kept." : "The worktree’s files will be removed. The Git branch will be kept."}</p><div class="wt-actions" style="justify-content:flex-end">${cancel}<button class="danger">${row.running ? "Stop and delete" : "Delete"}</button></div>`, "Rechecking worktree safety…")}</section>`;
    ...
    else if (m.view === "folder") body += `<div class="wt-notice error" role="alert" tabindex="-1">Folder rename is blocked: linked/main/common-directory or ancestor dependencies could break Git links, including unregistered checkouts. Stopping does not make this move safe. No files have moved.</div>${row.parentId ? '<p>Name follows the local branch; branch rename is out of scope.</p>' : link("Rename workspace instead", "rename", row.id)}`;
