...
function operations(contract: JsonObject): Map<string, { domain: ApiDomain; value: JsonObject }> {
  ...
  // inherited path-level one (OpenAPI merge semantics). Concatenating both
  // would make an unchanged contract compare against itself as breaking
  // whenever an override's schema differs from the inherited definition.
  const effectiveParameters = (inherited: unknown[], own: unknown[]): unknown[] => {
    const merged = new Map<string, unknown>();
    for (const entry of [...inherited, ...own]) {
      const resolved = object(resolveRefs(entry, contract, {}));
      merged.set(`${String(resolved.name)} ${String(resolved.in)}`, entry);
    }
  ...
  for (const [path, pathItem] of Object.entries(object(contract.paths))) {
    ...
    for (const [method, value] of Object.entries(object(pathItem))) {
      ...
      const operation = {
        ...operationValue,
        parameters: effectiveParameters(inheritedParameters, operationParameters),
