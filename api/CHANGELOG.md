...
- Added `hubUpdateWorkspaceDisplayName` (`POST /api/hub/workspaces/{workspaceId}/display-name`), valid while running or stopped; only the label changes. 400 is reserved for name validation; a registry persistence failure of a valid name is a documented retryable 500.
- Credential assignment mutations (`hubAssignCredential`, `hubUnassignCredential`, `hubDeleteCredential`, `hubAssignWorkspaceCredentials`) answer `409` while a pending onboarding journal awaits Hub recovery, so recovery can trust its recorded pre-commit assignment state. The unassign operation's inventory now also lists its documented `409`.
